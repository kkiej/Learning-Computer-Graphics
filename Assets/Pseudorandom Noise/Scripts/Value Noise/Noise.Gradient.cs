...
public static partial class Noise
{
    ...
    {
        public float4 Evaluate(SmallXXHash4 hash, float4 x) => default(TG).Evaluate(hash, x);

        public float4 Evaluate(SmallXXHash4 hash, float4 x, float4 y) => default(TG).Evaluate(hash, x, y);

        public float4 Evaluate(SmallXXHash4 hash, float4 x, float4 y, float4 z) => default(TG).Evaluate(hash, x, y, z);

        public float4 EvaluateAfterInterpolation(float4 value) => abs(default(TG).EvaluateAfterInterpolation(value));
    }
}